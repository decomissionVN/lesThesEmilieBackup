...
function display_Admin_functions(){
    ...
    if (token !== null && userId !== null) {
        ...
        document.getElementById('order_list_delivered').style.display = "none";
    }
}
...
function display_user_priviledges(response) {
    ...
    if (response.admin === "no"){
        ...
        document.getElementById('order_list_delivered').style.display = "block";
    }
}
...
function display_all_orders() {
    ...
    let userId = JSON.parse(window.localStorage.getItem('userId'));
        ...
        fetch('https://api.lesthesdemilie.net/api/orders/displayOrderDelivered', { method: "GET", headers: { 'Authorization': 'bearer ' + token.token_access }}).then(response => response.json()).then(response => display_all_orders_onpage(response))
        .catch(function (error) {
            console.log('GET ' + error.message)
        })
}
...
function display_all_orders_onpage(response){
    ...
    if (response.length !== 0){
        ...
        document.getElementById('order_list_delivered').innerHTML = listProducts;
    }
}
