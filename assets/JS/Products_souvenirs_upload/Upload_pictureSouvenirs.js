...
function display_all_products() {
    ...
    let userId = JSON.parse(window.localStorage.getItem('userId'));
        ...
        fetch('https://api.lesthesdemilie.net/api/pictures/picturesGetSouvenirs', { method: "GET" }).then(response => response.json()).then(response => display_all_products_onpage(response))
        .catch(function (error) {
            console.log('GET ' + error.message)
        })
}
...
function view_Product(_id){
    ...
    let queryParams = new URLSearchParams(window.location.search);
        ...
        window.setTimeout(window.location.replace("https://decommissionvn.github.io/lesThesEmilieBackup/Products/Preview_product" + "?" + queryParams.toString()), 5000);
}
