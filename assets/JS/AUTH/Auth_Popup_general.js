...
function Login_content() {
    ...
    if (language === 'fr') {
        let translation = { GetBack : "Recupérer le mot de passe", EnterEmail : "Renseignez votre mail", Forgot : "Mot de passe oublié", Welcome_Login : "Bienvenue sur votre espace des Thés d'Emilie", Signup : "Inscription", Password_Wrong : "Mot de passe incorrect", Password_Empty : "Veuillez renseigner un mot de passe", Login : "Connexion", Email : "Email", Warn_Email_Empty : "Veuillez renseigner votre Email", Warn_W_Email : "Aucun compte ne correspond à cette adresse", Password : "Mot de Passe" 
        }
        Login_content_translated(translation)
    } else {
        let translation = {  GetBack : "Get back my password", EnterEmail : "Enter your Email", Forgot : "Forgotten Password?", Welcome_Login : "Welcome to your account of Les thés d'Emilie", Signup : "Signup", Password_Wrong : "The password is incorrect", Password_Empty : "Please enter a password", Login : "Login", Email : "Email", Warn_Email_Empty : "Please enter your Email", Warn_W_Email : "No account was created with this Email", Password : "Password" 
        }
        Login_content_translated(translation)
    }
...
function Signup_content() {
    let language = localStorage.getItem("language");
    if (language === 'fr') {
        let translation = { Warn_Email_Valid : "Cet Email n'est pas valide", Password_Strong : "Pour votre sécurité, le mot de passe doit contenir 6 à 20 caractères dont un nombre, une majuscule et une minuscule", Signup : "Inscription", Agree_CGU : "Veuillez accepter les CGU", Read : "J'ai lu et j'accepte les ", CGU_Small : "CGU", CGU : "Conditions générales d'utilisation", Name_Warn : "Veuillez renseigner votre nom", Name : "Votre nom", Welcome_Signup : "Créez votre espace sur Les Thés d'Emilie", Password_Empty : "Veuillez renseigner un mot de passe", Login : "Connexion", Email : "Email", Warn_Email_Empty : "Veuillez renseigner votre Email", Warn_W_Email : "Aucun compte ne correspond à cette adresse", Password : "Mot de Passe" 
        }
        Signup_content_translated(translation)
    } else {
        let translation = { Warn_Email_Valid : "This Email is not valid", Password_Strong : "For your security, your password must be 6 to 20 characters and contain at least one numeric digit, one uppercase and one lowercase letter", Signup : "Signup", Agree_CGU : "Please agree with the CGU", Read : "I read and accept the ", CGU_Small : "CGU", CGU : "General Conditions of Use", Name_Warn : "Please let us know your name", Name : "Your name", Welcome_Signup : "Create your account on Les Thés d'Emilie",Password_Empty : "Please choose a password", Login : "Login", Email : "Email", Warn_Email_Empty : "Please enter your Email", Warn_Email_Used : "This Email is already used for another account", Password : "Password" 
        }
        Signup_content_translated(translation)
    }
    ...
    let form_signup = `<p>${translation.Welcome_Signup}</p>
                       <form method="post" id="contact" autocomplete="on">
                            ...
                            <p class="m-0"><input type="checkbox" name="CGU" id="CGU" placeholder="CGU" required>${translation.Read}<a id="CGU_link" href="../CGU/Cgu.html" alt="CGU Trois-Fils">${translation.CGU_Small}<a/></p>
...
function verify_stength_password(user) {
    ...
    if(user.password.match(passw)) { 
        Clear_Style_Signup_Form();
        fetch('https://api.lesthesdemilie.net/api/auth/signup', { method: "POST", headers: { 'Content-Type': "application/json" }, body: JSON.stringify(user) }).then(response => response.json()).then(response => user_saved_signup(response))
...
function Login() {
    ...
    fetch('https://api.lesthesdemilie.net/api/auth/login', { method: "POST", headers: { 'Content-Type': "application/json" }, body: JSON.stringify(user) }).then(response => response.json()).then(response => user_saved_login(response))
        .catch(function (error) {
...
function GetBack() {
    ...
    let password = document.getElementsByTagName("input");
        ...
        fetch('https://api.lesthesdemilie.net/api/auth/ResetPassword', { method: "POST", headers: { 'Content-Type': "application/json" }, body: JSON.stringify(user) }).then(response => response.json()).then(display('response'))
        .catch(function (error) {
            console.log('ERROR' + error.message)
        })
}
